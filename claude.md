  ...
  async (req, res) => {
    ...
    try {
      ...
      if (event.name === "transaction.approved") {
        // FedaPay place l'objet transaction dans event.data.object
        const transaction = event.data?.object ?? event.data;

        const donorName = transaction.customer
          ? `${transaction.customer.firstname ?? ""} ${transaction.customer.lastname ?? ""}`.trim()
          : "Donateur anonyme";
        const donorEmail = transaction.customer?.email ?? "inconnu@fedapay.com";
